...
async def pm_next_page(bot, query):
    ...
    if SHORT_URL and SHORT_API:          
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[🎬 [ File size ] ➳{get_size(file.🎬 [file_size] ➳)}] {file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}")),
                    InlineKeyboardButton(text=f"🎬 [ File size ] ➳{get_size🎬 [(file.file_size] ➳)}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=files_{file.file_id}"))] for file in files ]
    else:        
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[🎬 [ File size ] ➳{get_size(file.🎬 [file_size] ➳)}] {file.file_name}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
        else:
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", callback_data=f'pmfile#{file.file_id}'),
                    InlineKeyboardButton(text=f"🎬 [ File size ] ➳{get_size(file.🎬 [file_size] ➳)}", callback_data=f'pmfile#{file.file_id}')] for file in files ]
    ...
    if SHORT_URL and SHORT_API:          
        if SINGLE_BUTTON:
            ...
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}")),
                    InlineKeyboardButton(text=f"🎬 [ File size ] ➳{get_size(file.🎬 [file_size] ➳)}", url=await get_shortlink(f"https://telegram.dog/{temp.U_NAME}?start=pre_{file.file_id}"))] for file in files ]
            
    else:        
        if SINGLE_BUTTON:
            btn = [[InlineKeyboardButton(text=f"[🎬 [ File size ] ➳{get_size(file.🎬 [file_size] ➳)}] {file.file_name}", callback_data=f'{pre}#{file.file_id}')] for file in files ]
            
        else:
            btn = [[InlineKeyboardButton(text=f"{file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}'),
                    InlineKeyboardButton(text=f"🎬 [ File size ] ➳{get_size(file.🎬 [file_size] ➳)}", callback_data=f'{pre}#{file.file_id}')] for file in files ]
